...
import useFavouritesStore from "@/lib/favouritesStore";
...
function Avatar({ sender, size = 28 }) {
  const [failed, setFailed] = useState(false);
  const src = `/pfp/${sender}.jpg`;
  if (!sender) return null;
  return failed ? (
    <div
      className="inline-flex shrink-0 select-none items-center justify-center rounded-full bg-white/10 text-xs font-semibold text-gray-200 border border-white/10"
    ...
    <Image
      ...
      width={size}
      height={size}
      className="rounded-full border border-white/10 object-cover pointer-events-none"
...
export default function Home({ items, lastUpdated }) {
  ...
  const toggleFavouriteId = useFavouritesStore((s) => s.toggle);
  const favouritesSet = useMemo(() => new Set(favouriteIds), [favouriteIds]);

  const computed = useMemo(() => {
    const base = showFavourites
      ? items.filter((i) => favouritesSet.has(makeItemId(i)))
      : items;
  ...
  }, [items, activeType, showFavourites, favouritesSet]);
  ...
  return (
    <div className="font-sans min-h-screen">
      <header className="sticky top-0 z-40 pointer-events-auto backdrop-blur supports-[backdrop-filter]:bg-background/70 bg-background/90 border-b border-white/8">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="relative flex items-center gap-2">
              <button
                ...
                title={showFavourites ? "Show mentions" : "Show favourites"}
                aria-label={
                  showFavourites ? "Show mentions" : "Show favourites"
                }
                aria-pressed={showFavourites}
                className={`inline-flex items-center justify-center rounded-md border h-[34px] px-3 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 transition-colors ${
                  showFavourites
        ...
        {computed.list.length === 0 ? (
          <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
            {showFavourites ? (
              <>
                ...
                <p className="text-lg font-semibold">No favourites yet</p>
                <p className="text-gray-400 mt-1 text-sm">
                  Star items to save them here for quick access
                </p>
              </>
            ) : (
              <>
          ...
          >
            {computed.list.map((item) => {
              ...
              const isFav = favouritesSet.has(id);
              return (
                <article
                  key={id}
                  className="rounded-lg border border-white/10 bg-black/40 p-4 shadow-sm relative"
                  aria-labelledby={`rec-${id}-title`}
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col">
                      ...
                      <button
                        type="button"
                        onClick={() => toggleFavouriteId(id)}
                        aria-pressed={isFav}
                        aria-label={isFav ? "Unfavourite" : "Favourite"}
                        className={`absolute bottom-0 right-0 inline-flex items-center gap-1 border-t border-l rounded-br-lg rounded-tl-lg text-center py-0.5 px-2.5 text-sm font-medium backdrop-blur-xs ${
                          isFav
                      ...
                      >
                        <Star
                          size={14}
                          className={`shrink-0 ${isFav ? "fill-current" : ""}`}
                        />
